...
#feedback[
  $cal(G)_2 = sigma({H H}, {H T, T H}, {T T})$.  If an exam question asks for "write down the $sigma$-algebra", need to write the set out fully.
]
...
#feedback[
  Correct (intended) way is to write the event as a function of $X$ (for $cal(F)$) and $S$ (for $cal(G)$) -- enumerating all elements is impossible when $Omega$ is not finite e.g.~when $Omega := {H, T}^n quad n in NN$.
  
  $A_2 &:= { "at least one" H } \
       &= { omega in Omega : X_1(omega) = H or X_2(omega) = H } \
       &= union_(i=1)^2 X^(-1)_i (H) quad in cal(F)_2.
  $
  
  $A_2 = { S_2 >= 1 } in cal(G)_2 = sigma(S_2).$
]
...
#feedback[
  Use the notation $X_1^(-1)(H)$ for simplicity.

  The intuition does not function as a full mathematical proof.
]
...
#feedback[Only need to prove one of $Omega$ and $diameter$ in $cal(F)$, thanks to the complement rule -- less to prove, less chance of error.]
...
#feedback[No need to prove the $P$-a.s.~uniqueness -- it is part of the definition given here.]
...
#attempt[
  ...
  Using _lemma 2_, $Z = E[X|cal(G)]$ is measurable in $cal(G)$ and therefore must be $P$-a.s.~constant $c$.  Take $A = Omega in cal(G)$:
